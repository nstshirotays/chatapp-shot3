...
$(function () {
	$("#imageFiles").on("change", function (evt) {
		...
		if (!targetFile.type.match(/image/)) {
			document.getElementById("error").innerHTML = '画像を選択してください';
			return;
		}
...
function readJPEGFile(evt) {
	...
	var soi = String.fromCharCode(0xFF, 0xD8); // SOI スタートマーカ (Start of Image) 　0xFFD8
	var head = bin.substr(0, 2);

	if (soi != head) {
		document.getElementById("error").innerHTML = "JPEGを指定してください";
		return;
	}
	...
	reader.onload = function (e) {
		...
		image.onload = function () {
			...
			if (height > 1000 && width > 1000) {
				document.getElementById("error").innerHTML = 'サイズは1000 x 1000以下にしてください';
				return;
			} else {
			    if (bin.length > 10000000) {
    				document.getElementById("error").innerHTML = 'ファイル容量は10mbまでです';
    				return;
